        ...
        <tbody>
            ...
            @if (!$items->isEmpty())
                ...
                <tr>
                    ...
                    <td>{{$item->id}}</td>
                    <td>{{$item->name}}</td>
                    <td><a href="{{route('admin::menucat.edit', ['id' => $item->id])}}" class="btn btn-sm btn-primary"><i class="fa fa-long-arrow-right"></i></a></td>
                    <td>{{$item->slug}}</td>
                    <td>
                        <a href="{{route('admin::menucat.edit', ['id' => $item->id])}}" 
            ...
            @else
            <tr>
                <td colspan="6" class="text-center"><h4>{{ trans('admin::message.not_found_items') }}</h4></td>
            </tr>
            @endif
        </tbody>
    </table>

        ...
        <tbody>
            ...
            @if (!$items->isEmpty())
                ...
                <tr>
                    ...
                    <td>{{$item->name}}</td>
                    <td>{{$item->slug}}</td>
                    <td><a href="{{route('admin::post.index', ['tags' => [$item->id], 'status' => 1])}}">{{$item->count}}</a></td>
                    <td>
                        <a href="{{route('admin::tag.edit', ['id' => $item->id])}}" class="btn btn-sm btn-info" title="{{trans('admin::view.edit')}}"><i class="fa fa-edit"></i></a>
                    </td>
                </tr>
                @endforeach
            @else

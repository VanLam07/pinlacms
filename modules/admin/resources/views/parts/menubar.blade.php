...
<aside class="main-sidebar">
    ...
    <section class="sidebar">
        ...
        <?php
        if (function_exists('nestedAdMenus')) {
            function nestedAdMenus($items, $depth = 0) {
                $html = '<ul class="' . (($depth == 0) ? 'sidebar-menu' : 'treeview-menu') . '">';
                foreach ($items as $item) {
                    if (canDo($item['cap'])) {
                        $routeParams = (isset($item['route_params'])) ? $item['route_params'] : [];
                        $hasChilds = (isset($item['childs']) && $item['childs']);
                        $html .= '<li class="' . isSubActive($item['route']) . ' '.($hasChilds ? 'has-sub' : '').'">';
                        $html .= '<a href="' . route($item['route'], $routeParams) . '" title="'.$item['name'].'"><i class="fa ' . (isset($item['icon']) ? $item['icon'] : 'fa-circle') . '"></i> <span>' . $item['name'] . '</span> ' . ($hasChilds ? '<b></b>' : '') . '</a>';
                        if ($hasChilds) {
                            $html .= nestedAdMenus($item['childs'], $depth + 1);
                        }
                        $html .= '</li>';
                    }
                }
